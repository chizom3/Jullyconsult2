				...
				if (W) {
					...
					v.forEach(function (d) {
						...
						if (e) {
							...
							m.setAttribute(
								"as",
								"IFRAME" == d.tagName ? "document" : "script",
							);
					...
					if (3 == z) {
						...
						function n() {
							for (
								...
								for (; (x = seraph_accel_izrbpb.a[p].shift());)
									if (x(n)) return;
					...
					if (void 0 !== D) {
						...
						if (k) {
							...
							for (var n = a.target; n; n = n.parentNode)
								if (n.getAttribute) {
									if (p(n, a.type, "data-lzl-clk-no") || x(n, a.type, V))
										f = !1;
